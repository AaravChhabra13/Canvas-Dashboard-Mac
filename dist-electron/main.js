...
function buildURL(url2, params, options) {
  ...
  const _options2 = utils$1.isFunction(options) ? {
    serialize: options
  } : options;
  const serializeFn = _options2 && _options2.serialize;
  let serializedParams;
  if (serializeFn) {
    serializedParams = serializeFn(params, _options2);
  } else {
    serializedParams = utils$1.isURLSearchParams(params) ? params.toString() : new AxiosURLSearchParams(params, _options2).toString(_encode);
  }
  if (serializedParams) {
  ...
  return wrapAsync(async function dispatchHttpRequest(resolve2, reject, onDone) {
    ...
    if (config.socketPath) {
      ...
      if (config.allowedSocketPaths != null) {
        ...
        const resolvedSocket = require$$1$2.resolve(config.socketPath);
        const isAllowed = allowed.some(
          (entry) => typeof entry === "string" && require$$1$2.resolve(entry) === resolvedSocket
        );
        if (!isAllowed) {
          return reject(new AxiosError$1(
    ...
    req = transport.request(options, function handleResponse(res) {
      ...
      if (responseType === "stream") {
        if (config.maxContentLength > -1) {
          const limit2 = config.maxContentLength;
    ...
    if (utils$1.isStream(data)) {
      ...
      if (config.maxBodyLength > -1 && config.maxRedirects === 0) {
        const limit2 = config.maxBodyLength;
        let bytesSent = 0;
...
function saveToken(token) {
  if (!electron.safeStorage.isEncryptionAvailable()) throw new Error("safeStorage unavailable");
  const buf = electron.safeStorage.encryptString(token);
...
function getToken() {
  ...
  try {
    return electron.safeStorage.decryptString(Buffer.from(hex, "hex"));
  } catch {
    return null;
  }
}
...
function colorForIndex(i) {
  return COURSE_COLORS$2[i % COURSE_COLORS$2.length];
}
async function fetchCoursesFromApi(baseUrl, token) {
  const resp = await axios.get(`${baseUrl}/api/v1/courses`, {
    params: { enrollment_state: "active", per_page: 50 },
    headers: { Authorization: `Bearer ${token}` },
    timeout: 15e3
  });
  return resp.data.filter((c) => c && c.id && c.name).map((c, i) => ({
    id: String(c.id),
    name: c.name,
...
async function fetchPlannerItems(baseUrl, token, lookaheadDays, courses) {
  var _a, _b, _c, _d, _e, _f;
  const now = /* @__PURE__ */ new Date();
  const endDate = new Date(now);
  endDate.setDate(endDate.getDate() + Math.min(lookaheadDays, 90));
  const courseMap = new Map(courses.map((c) => [c.id, c]));
  const resp = await axios.get(`${baseUrl}/api/v1/planner/items`, {
    params: {
      per_page: 100,
  ...
  for (const item of resp.data) {
    try {
      ...
      const courseName = (course == null ? void 0 : course.name) ?? item.context_name ?? "Unknown Course";
      const courseColor = (course == null ? void 0 : course.color) ?? colorForIndex(parseInt(courseId, 10) || 0);
      const dueAt = ((_a = item.plannable) == null ? void 0 : _a.due_at) ?? null;
      const submitted = ((_b = item.submissions) == null ? void 0 : _b.submitted) ?? false;
      const graded = ((_c = item.submissions) == null ? void 0 : _c.graded) ?? false;
      const submissionState = graded ? "graded" : submitted ? "submitted" : "unsubmitted";
      result.push({
        id: `rest-${item.plannable_type}-${item.plannable_id}`,
        title: ((_d = item.plannable) == null ? void 0 : _d.title) ?? "Untitled",
        courseId,
        courseName,
        courseColor,
        dueAt,
        ...
        submissionState,
        pointsPossible: ((_e = item.plannable) == null ? void 0 : _e.points_possible) ?? null,
        canvasUrl: item.html_url ?? ((_f = item.plannable) == null ? void 0 : _f.html_url) ?? "",
...
function plannerTypeToType(t2) {
  if (t2 === "quiz") return "quiz";
  if (t2 === "discussion_topic") return "discussion";
  if (t2 === "announcement") return "announcement";
  return "assignment";
}
const COURSE_COLORS$1 = [
  "#6366f1",
  "#f59e0b",
  "#10b981",
  "#ef4444",
...
function courseColorFromName$1(name) {
  let h = 0;
  for (let i = 0; i < name.length; i++) h = h * 31 + name.charCodeAt(i) >>> 0;
  return COURSE_COLORS$1[h % COURSE_COLORS$1.length];
}
function extractCsrfToken(cookie) {
  const match = cookie.match(/_csrf_token=([^;]+)/);
  return match ? decodeURIComponent(match[1]) : void 0;
}
function mapSubmissionState(state2) {
  if (state2 === "submitted") return "submitted";
  if (state2 === "graded") return "graded";
  if (state2 === "unsubmitted" || state2 === "pending_review") return "unsubmitted";
  return "unknown";
}
const GQL_QUERY = `{
  allCourses {
...
async function fetchAssignmentsViaGraphQL(baseUrl, cookie, lookaheadDays, storedCourses) {
  var _a, _b, _c;
  ...
  const gqlCourses = (_b = (_a = resp.data) == null ? void 0 : _a.data) == null ? void 0 : _b.allCourses;
  ...
  for (const gqlCourse of gqlCourses) {
    ...
    const course = stored ?? {
      id: courseId,
      name: gqlCourse.name,
      color: courseColorFromName$1(gqlCourse.name),
      hidden: false
    };
    courses.push(course);
    if (course.hidden) continue;
    for (const node2 of gqlCourse.assignmentsConnection.nodes) {
      if (node2.dueAt && new Date(node2.dueAt) > cutoff) continue;
      const submissionState = mapSubmissionState((_c = node2.submissionsConnection.nodes[0]) == null ? void 0 : _c.workflowState);
      const isOverdue = node2.dueAt !== null && new Date(node2.dueAt) < now && submissionState === "unsubmitted";
      assignments2.push({
        id: `gql-${node2._id}`,
        title: node2.name,
        courseId,
        ...
        type: "assignment",
        submissionState,
        pointsPossible: node2.pointsPossible,
        canvasUrl: node2.htmlUrl,
        isOverdue,
        source: "graphql"
      });
    }
  }
  return { assignments: assignments2, courses };
}
...
const ICAL = _require("ical.js");
function mergeCourses(fresh, stored) {
...
async function syncAll(baseUrl, icalUrl, sessionCookie, lookaheadDays, storedCourses) {
  ...
  if (token) {
    ...
    const assignments2 = await fetchPlannerItems(baseUrl, token, lookaheadDays, mergedCourses);
    return { assignments: assignments2, courses: mergedCourses };
  }
  if (sessionCookie) {
    try {
  ...
  if (icalUrl) {
    const assignments2 = await fetchAssignmentsFromIcal(icalUrl);
    return { assignments: assignments2, courses: [] };
  }
...
function courseColorFromName(name) {
  let h = 0;
  for (let i = 0; i < name.length; i++) h = h * 31 + name.charCodeAt(i) >>> 0;
  return COURSE_COLORS[h % COURSE_COLORS.length];
}
async function fetchAssignmentsFromIcal(icalUrl) {
  ...
  for (const vevent of vevents) {
    try {
      ...
      const description2 = event.description || "";
...
function checkAndFireNotifications(assignments2, leadTimes, lastSyncAt) {
  if (!electron.Notification.isSupported()) return;
  const now = /* @__PURE__ */ new Date();
  const prevSync = lastSyncAt ? new Date(lastSyncAt) : /* @__PURE__ */ new Date(0);
  for (const a of assignments2) {
    ...
    for (const leadMinutes of leadTimes) {
      const fireAt = new Date(dueDate.getTime() - leadMinutes * 6e4);
      const key = `${a.id}:${leadMinutes}`;
      if (fired.has(key)) continue;
      if (fireAt <= prevSync || fireAt > now) continue;
      fired.add(key);
      const hours = leadMinutes / 60;
      const timeLabel = leadMinutes % 60 === 0 ? `${hours} hour${hours !== 1 ? "s" : ""}` : `${leadMinutes} minutes`;
      const n = new electron.Notification({
        title: a.courseName,
        body: `${a.title} — due in ${timeLabel}`,
        silent: false
      });
      const url2 = a.canvasUrl;
      if (url2) n.on("click", () => electron.shell.openExternal(url2));
      n.show();
    }
  }
}
...
let store;
let assignments = [];
let syncState = { status: "idle", lastSyncAt: null };
let syncTimer = null;
...
async function runSync(win, callbacks) {
  ...
  try {
    const storedCourses = store.get("courses");
    ...
    syncState = { status: "idle", lastSyncAt: (/* @__PURE__ */ new Date()).toISOString() };
...
function startSyncTimer(win, callbacks) {
  if (syncTimer) clearInterval(syncTimer);
  const intervalMs = getSettings().syncIntervalMinutes * 6e4;
  syncTimer = setInterval(() => runSync(win, callbacks), intervalMs);
}
function setupIPC(win, callbacks) {
  ...
  electron.ipcMain.handle("assignments:get", () => assignments);
  electron.ipcMain.handle("sync:status:get", () => syncState);
  electron.ipcMain.on("sync:trigger", () => runSync(win, callbacks));
  electron.ipcMain.handle("sync:run-and-wait", async () => {
  ...
  electron.ipcMain.handle("settings:set", async (_event, patch2) => {
    const current = getSettings();
    const updated = { ...current, ...patch2 };
    store.set("settings", updated);
    if (patch2.syncIntervalMinutes !== void 0) startSyncTimer(win, callbacks);
    const urlChanged = patch2.canvasIcalUrl !== void 0 && patch2.canvasIcalUrl !== current.canvasIcalUrl || patch2.canvasBaseUrl !== void 0 && patch2.canvasBaseUrl !== current.canvasBaseUrl || patch2.canvasSessionCookie !== void 0 && patch2.canvasSessionCookie !== current.canvasSessionCookie;
  ...
  electron.ipcMain.handle("token:save", (_event, token) => {
    saveToken(token);
    return true;
  });
  electron.ipcMain.handle("token:check", () => hasToken());
  electron.ipcMain.handle("token:delete", () => {
    deleteToken();
    return true;
  });
  ...
  electron.ipcMain.handle("completed:get", () => store.get("completedIds"));
  electron.ipcMain.handle("completed:toggle", (_event, id2) => {
    const ids = store.get("completedIds");
    const updated = ids.includes(id2) ? ids.filter((x) => x !== id2) : [...ids, id2];
    store.set("completedIds", updated);
    return updated;
  });
  electron.ipcMain.handle("tasks:get", () => store.get("personalTasks"));
  electron.ipcMain.handle("tasks:save", (_event, tasks) => {
    store.set("personalTasks", tasks);
    return tasks;
  });
  electron.ipcMain.handle("notifications:request", () => {
    requestNotificationPermission();
    return true;
  });
  electron.ipcMain.handle("onboarding:complete", async () => {
    const current = getSettings();
...
function createPanel() {
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    panel.once("ready-to-show", () => {
      panel == null ? void 0 : panel.show();
      panel == null ? void 0 : panel.webContents.openDevTools({ mode: "detach" });
    });
  } else {
    panel.loadFile(path$1.join(RENDERER_DIST, "index.html"));
  }
  panel.on("blur", () => {
    lastBlurTime = Date.now();
    panel == null ? void 0 : panel.hide();
  });
  panel.webContents.on("before-input-event", (_ev, input) => {
    if (input.type === "keyDown" && input.key === "Escape") panel == null ? void 0 : panel.hide();
  });
}
function createOnboardingWindow() {
  onboardingWin = new electron.BrowserWindow({
    width: 560,
    height: 480,
    show: false,
    frame: false,
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    onboardingWin.once("ready-to-show", () => {
      onboardingWin == null ? void 0 : onboardingWin.show();
      onboardingWin == null ? void 0 : onboardingWin.center();
    });
  } else {
    onboardingWin.loadFile(path$1.join(RENDERER_DIST, "index.html"), { hash: "onboarding" });
    onboardingWin.once("ready-to-show", () => {
      onboardingWin == null ? void 0 : onboardingWin.show();
      onboardingWin == null ? void 0 : onboardingWin.center();
    });
...
electron.app.whenReady().then(() => {
  ...
  electron.powerMonitor.on("resume", () => {
    ...
    panel == null ? void 0 : panel.webContents.send("panel:shown");
  });
