...
function getErrorPath$1(dataProp, dataPropType, jsPropertySyntax) {
  if (dataProp instanceof codegen_1$_.Name) {
    ...
    return jsPropertySyntax ? isNumber2 ? (0, codegen_1$_._)`"[" + ${dataProp} + "]"` : (0, codegen_1$_._)`"['" + ${dataProp} + "']"` : isNumber2 ? (0, codegen_1$_._)`"/" + ${dataProp}` : (0, codegen_1$_._)`"/" + ${dataProp}.replace(/~/g, "~0").replace(/\\//g, "~1")`;
  }
...
function requireDefaults() {
  ...
  defaults$2.assignDefaults = assignDefaults2;
...
const tokenStore = new ElectronStore({
  name: "token",
...
function colorForIndex(i) {
  return COURSE_COLORS$1[i % COURSE_COLORS$1.length];
}
...
async function syncAll(baseUrl, icalUrl, lookaheadDays, storedCourses) {
...
function setupIPC(win, callbacks) {
  ...
  electron.ipcMain.on("sync:trigger", () => runSync(win));
  ...
  electron.ipcMain.handle("settings:set", async (_event, patch2) => {
    ...
    if (patch2.syncIntervalMinutes !== void 0) startSyncTimer(win);
    const urlChanged = patch2.canvasIcalUrl !== void 0 && patch2.canvasIcalUrl !== current.canvasIcalUrl || patch2.canvasBaseUrl !== void 0 && patch2.canvasBaseUrl !== current.canvasBaseUrl;
